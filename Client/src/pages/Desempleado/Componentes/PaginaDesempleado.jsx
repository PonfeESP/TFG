...
export const PaginaDesempleado = () => {
    ...
    useEffect(() => {
        axios({
            ...axiosConfig,
            url: 'http://localhost:8000/Mostrar/Ofertas',
            method: 'GET'
        })
            .then(res => {
                setOfertas(res.data);
    ...
    console.log("logoutError:bbbbbbbbbbbb", ofertas);
                    ...
                    <TableBody>
                        {ofertas.map((oferta, index) => (
                            <TableRow key={oferta._id}>
                                ...
                                </TableCell>                                <TableCell className="degradado-invertido" style={{ background: 'transparent' }}><Typography sx={{ fontWeight: 'bold', color: 'white' }}>{oferta.Descripcion}</Typography></TableCell>
